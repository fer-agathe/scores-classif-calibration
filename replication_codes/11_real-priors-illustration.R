...
# If any categorical variable, one-hot encoding
...
# Smoothing parameter for continuous variables
spline_df <- 6
num_nb_val <- map_dbl(
  num_names,
  ~data_dmy$train |> pull(.x) |> unique() |> length()
)
deg_num <- ifelse(num_nb_val < spline_df, num_nb_val, spline_df)

# String for the numerical terms that enter in the formula:
...
# Predicted scores (again, but using the function):
...
# Integer variables cause gamsel to crash. We change those to doubles
...
# Predicted scores (using the wrapper function):
...
# To avoid splines to crash for discrete variables
