# Generalized Linear Models {#sec-simul-glm}


:::{.callout-note}

This chapter examines the scores returned by a logistic model under the different scenarios.
...
As in previous chapters, we define a function to run replications of the simulations for each scenario. This function is called `simul_glm()`{.R}. It uses multiple helper functions also defined here.
...
```{r define-simul_glm}
#| code-fold: true
#| code-summary: The `simul_glm()`{.R} function
...
```{r export-cluster-and-run-glm-simul, eval=FALSE}
#| code-fold: true
...
```{r}
#| fig-cap: Metrics for the GLM model computed on 100 replications of the simulations for each scenario.
#| label: fig-bp-glm
#| code-fold: true
...
```{r}
#| fig-cap: Distribution of true probabilities and estimated scores on test set for the logistic regression.
#| label: fig-dist-scores-glm
#| code-fold: true
#| fig-height: 8
#| fig-width: 12
...
pdf(
  file =str_c( "figures/bp_synthetic_glm.pdf"), 
  width = 12, height = 8
)
plot_hist_dgp(repn = 1)
dev.off()
