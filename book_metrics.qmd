...
In the `dispersion_metrics()`{.R} that we define to that end, we consider two values for the number of bins: $m\in \{10,20\}$. We also consider switching the reference distribution (where $Q$ denotes the distribution of the true probabilities and $P$ denotes the distribution of scores).
