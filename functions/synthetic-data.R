...
#'   the following function \deqn{p_i =
#'   \left(\frac{1}{1+\exp(-\eta_i)}\right)^{\alpha},} where
#'   \eqn{\eta_i} is defined as follows: \deqn{\eta_i =\left( a_1 x_1 + a_2 x_2 + a_3 x_3 \right),} where \eqn{(a_1,
#'   a_2, a_3) = (4, 3, 2)} by default.
#'
#' @param n_obs number of desired observations
#' @param seed seed to use to generate the data
#' @param a vector of length 3 with values for \eqn{a}. Default to \eqn{(a_1,
#'   a_2, a_3) = (4, 3, 2)}
#'
...
#'   \item{\code{x1, x2, x3, x4}}{Features, where \code{x4} is noise}
#' }
#' @examples
...
sim_data <- function(n_obs = 2000,
  ...
  if (length(a) != 3) stop("`a` must be a vector of numerical of length 2")
  ...
  # True latent score
  eta <- a1*x1 + a2*x2 + a3*x3 - (a1+a2+a3)/3
...
#' Get train/test samples from the DGP
#'
#' @param n_obs number of desired observations
...
#'   a_2, a_3) = (4, 3, 2)} in the DGP (see \code{\link{sim_data}})
#'
...
get_samples <- function(n_obs = 2000,
  ...
  tb_test <- data |> slice(-train_index)

  true_probas_train <- true_probas |> slice(train_index)
  true_probas_test <- true_probas |> slice(-train_index)
