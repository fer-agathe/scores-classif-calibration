...
simulate_data <- function(n_num = 2,
  ...
  if (linear_predictor == TRUE) {
    n_obs <- size_train + size_test
  } else {
    n_obs <- (size_train + size_test) * linear_predictor_factor
  }
  ...
  if (size_test > 0) {
    tb_test <- tb |> dplyr::slice((size_train + 1):n_obs)
    true_prob_test = true_prob[(size_train + 1):n_obs]
  } else {
    tb_test <- NULL
    true_prob_test <- NULL
  }
...
#'          simulation scenario.
#'  - data: list with the simulated data (train, test, probs_train and probs_test)
